            ...
            '#wbApp .c-bk-pick.open{max-height:300px;padding:0 16px 14px;overflow-y:auto;scrollbar-width:none}',
            '#wbApp .c-bk-pick.open::-webkit-scrollbar{display:none}',
(function () {
    'use strict';
    ...
    ══════════════════════════════════════ */
    // 清空旧 localStorage 数据
    localStorage.removeItem('wb-books');
    localStorage.removeItem('wb-entries');

    function loadBooks() {
        try { return JSON.parse(localStorage.getItem('byeol-wb-books') || '[]'); } catch(e) { return []; }
    }
    function saveBooks(b) { localStorage.setItem('byeol-wb-books', JSON.stringify(b)); }
    function loadEntries() {
        try { return JSON.parse(localStorage.getItem('byeol-wb-entries') || '[]'); } catch(e) { return []; }
    }
    function saveEntries(e) { localStorage.setItem('byeol-wb-entries', JSON.stringify(e)); }
    ...
    window.WB = {
        ...
        getInjection: function (entId, position) {
            /* 每次调用时从 localStorage 实时读取，确保即使未打开 World Book 也能正确注入 */
            var liveEntries = entries;
            if (!liveEntries || !liveEntries.length) {
                try { liveEntries = JSON.parse(localStorage.getItem('wb-entries') || '[]'); } catch(e) { liveEntries = []; }
                if (liveEntries.length) entries = liveEntries;
            }
            var relevant = liveEntries.filter(function(e) {
                return e.enabled && e.charIds && e.charIds.length > 0 && e.charIds.indexOf(entId) !== -1
                    && e.position === position;
            });
            relevant.sort(function(a,b){ return (a.insertionOrder||0)-(b.insertionOrder||0); });
            return relevant.map(function(e){
                var text = e.content || '';
                if (e.fileContent && e.fileContent !== text) {
                    text += '\n\n' + e.fileContent;
                }
                return text;
            }).join('\n\n');
    ...
    function buildCSS() {
        return [
            ...
            /* status bar */
            '#wbApp .wb-sb{position:relative;z-index:10;height:44px;padding:0 36px 0 18px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;visibility:hidden}',
            ...
            '#wbApp .c-books{font-family:"Space Grotesk",sans-serif;font-size:11px;color:var(--ink45);overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;line-height:1.5;max-height:33px}',
            ...
            '#wbApp .c-bk-pick{padding:0 16px;max-height:0;overflow:hidden;transition:max-height .4s cubic-bezier(.16,1,.3,1),padding .3s}',
            '#wbApp .c-bk-pick.open{max-height:2000px;padding:0 16px 14px;overflow-y:auto;scrollbar-width:none}',
            '#wbApp .c-bk-pick.open::-webkit-scrollbar{display:none}',
            '#wbApp .c-bk-div{height:1px;background:var(--ink05);margin-bottom:12px}',
            '#wbApp .c-bk-row{display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--ink05);gap:10px;min-height:0}',
            '#wbApp .c-bk-row>div:first-child{flex:1;min-width:0;overflow:hidden}',
            '#wbApp .c-bk-name{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
    ...
    function entryCardHTML(e, dim) {
       ...
       function loadEntitiesDirect(cb) {
        try {
            var req = indexedDB.open('ByeolPhone_DB', 2);
            ...
            req.onsuccess = function(e) {
                var db = e.target.result;
                if (!db.objectStoreNames.contains('contacts')) { db.close(); cb([]); return; }
                var tx = db.transaction('contacts', 'readonly');
                var store = tx.objectStore('contacts');
                var getReq = store.getAll();
                getReq.onsuccess = function(ev) {
                    var contacts = ev.target.result || [];
                    db.close();
                    // 转换为世界书期望的格式
                    var ents = contacts.map(function(c) {
                        return {
                            id: c.id,
                            name: c.name || '未命名',
                            nickname: c.name,
                ...
                getReq.onerror = function() { db.close(); cb([]); };
            };
